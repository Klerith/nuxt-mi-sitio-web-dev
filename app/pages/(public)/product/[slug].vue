...
const { data, product, error, status } = await useProduct(slug);

// const { data: productSuggestions, status } = useLazyFetch(
//   `/api/product/${slug}/suggestions`
// );

// Explicar esto y luego el solucionarlo de forma elegante
...
// const isLoadingSuggestions = computed(() => status.value === 'pending');
// console.log({ isLoadingSuggestions: isLoadingSuggestions.value });

if (error.value) {
  navigateTo('/404');
  // throw createError({
  //   statusCode: 500,
  //   statusMessage: error.value?.message ?? 'Error al obtener el producto',
  // });
}
...
<template>
  <div v-if="product" class="container mx-auto px-4 py-8">
    ...
    <div v-if="product" class="mt-16">
      ...
      <!-- Skeleton con productos relacionados -->
